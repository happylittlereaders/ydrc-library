...
# ==========================================
# 4. Data Loading (MAPPED TO YOUR NEW LINK)
# ==========================================
...
def load_data():
    try:
        ...
        # Column Mapping for the new sheet structure
        c = {
            "il": 0, "rec": 1, "title": 2, "author": 4, "quiz": 5,
            "ar": 6, "word": 7, "fnf": 8, "topic": 9, "series": 10,
            "en": 11, "cn": 12
        }
        
        # Numeric Cleanups
        df.iloc[:, c['ar']] = pd.to_numeric(df.iloc[:, c['ar']], errors='coerce').fillna(0.0)
...
with st.sidebar:
    ...
    if not st.session_state.logged_in:
        ...
        with auth_mode[1]:
            ...
            if st.button("📝 Register"):
                if validate_email(r_email):
                    if len(r_pass) >= 6: register_user(r_email, r_pass, r_nick)
        ...
        st.markdown(f"<div class='user-badge {role_cls}'>{role_badges.get(st.session_state.user_role, 'Guest')}</div>", unsafe_allow_html=True)
        st.markdown(f"Hello, <b>{st.session_state.user_nickname}</b>", unsafe_allow_html=True)
...
def save_db_comment(book_title, text, comment_id=None):
    ...
    if comment_id: db.collection("comments").document(comment_id).update({"text": text, "time": data["time"]})
    else: db.collection("comments").add(data)

def delete_comment(comment_id):
    if db: db.collection("comments").document(comment_id).delete()
...
if st.session_state.bk_focus is not None:
    ...
    # Specific Scalar Access for word count to avoid Series error
    w_count = int(row.iloc[idx['word']])
    
    infos = [
        ("👤 Author", row.iloc[idx['author']]), ("📚 Genre", row.iloc[idx['fnf']]), ("🎯 Interest Level", row.iloc[idx['il']]),
        ("📊 AR Level", row.iloc[idx['ar']]), ("🔢 Quiz No.", row.iloc[idx['quiz']]), ("📝 Word Count", f"{w_count:,}"),
    ...
    if lb1.button("CN 中文理由"): st.session_state.lang_mode = "CN"; st.rerun()
    if lb2.button("US English"): st.session_state.lang_mode = "EN"; st.rerun()
    ...
    for i, m in enumerate(cloud_comments):
        ...
        st.markdown(f'<div class="comment-box"><div class="comment-meta"><span>👤 {m.get("author_nick")}</span><span>📅 {m.get("time")}</span></div>{m.get("text")}</div>', unsafe_allow_html=True)
        ...
        if st.session_state.logged_in and (is_mine or is_admin) and st.session_state.edit_id is None:
             if col_ops[1].button("🗑️", key=f"del_{i}"): delete_comment(m["id"]); st.rerun()

    if st.session_state.logged_in:
        is_editing = st.session_state.edit_id is not None
        with st.form("comment_form", clear_on_submit=False):
            st.write("✍️ " + ("Edit" if is_editing else "Post"))
            user_input = st.text_area("Content", value=st.session_state.temp_comment, key=f"in_{st.session_state.form_version}")
            cb1, cb2, _ = st.columns([1, 1, 4])
            if cb1.form_submit_button("Submit"):
                if user_input.strip():
...
# ==========================================
    ...
    tab1, tab2, tab3 = st.tabs(["📚 Gallery", "📊 Stats", "🏆 Favorites"])
    with tab1:
        if st.button("🎁 Mystery Box", use_container_width=True):
        ...
        if st.session_state.blind_idx is not None:
            ...
            st.info(f"Mystery: 《{b_row.iloc[idx['title']]}》")
            if st.button("🚀 View Details"): st.session_state.bk_focus = st.session_state.blind_idx; st.rerun()
            ...
            with cols[i % 3]:
                ...
                <div class="book-tile">
                    ...
                    <div class="tag-container">
                        ...
                        <span class="tag tag-word">{w_tag} Words</span>
                ...
                if cl.button("❤️" if voted else "🤍", key=f"h_{orig_idx}"):
                    ...
                    st.rerun()
                if cr.button("Details", key=f"d_{orig_idx}"): st.session_state.bk_focus = orig_idx; st.rerun()

    with tab2: st.bar_chart(f_df.iloc[:, idx['ar']].value_counts().sort_index())
    with tab3:
        if st.session_state.voted:
            title_to_idx = {str(r.iloc[idx['title']]): i for i, r in df.iterrows()}
            for b_name in st.session_state.voted:
                st.write(f"⭐ **{b_name}**")
                if st.button("Go", key=f"fav_{b_name}"): st.session_state.bk_focus = title_to_idx[b_name]; st.rerun()
        else: st.info("No favorites yet. Go and click ❤️!")
